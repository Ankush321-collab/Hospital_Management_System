      ...
      <div className='bg-gradient-to-b from-[#1e1e1e] to-[#121212] text-white w-full max-w-md rounded-2xl p-8 shadow-xl border border-gray-800'>
        ...
        {inputFields.map((field, index) => (
          <div className={`mb-${field.name === "password" ? "6" : "4"} relative`} key={index}>
            ...
            {field.type === "select" ? (
              <select
                className='w-full bg-[#2a2a2a] border border-gray-700 rounded-lg px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-[#7a6ff0] focus:border-transparent transition-all duration-200'
                name={field.name}
                value={field.value}
